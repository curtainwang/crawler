...
IParser = URLPaser()
socket = urllib.urlopen("http://edu.china.com/zh_cn/current/")#打开这个网页

#fout = file('qq_art_urls.txt','w')#要把这个链接写到这个文件中
IParser.feed(socket.read())#分析啦

reg = 'http://edu.china.com/.*'#这个是用来匹配符合条件的链接，使用正则表达式匹配
...
for url in IParser.urls:#链接都存在urls里
    ...
    if pattern.match(url):
        ...
        #url = url.replace(".html","_all.html#page_2")
            ...
            if len(artical)<>0:
                  i = i + 1
                  f = open("china_news/china_education_test/"+str(i) + '.txt','a+')
                  f.write(artical)
                  f.close()


IParser = URLPaser()
socket = urllib.urlopen("http://edu.china.com/zh_cn/current/index_1.html")#打开这个网页
IParser.feed(socket.read())#分析啦
reg = 'http://edu.china.com/.*'#这个是用来匹配符合条件的链接，使用正则表达式匹配
pattern = re.compile(reg)
for url in IParser.urls:#链接都存在urls里
