...
IParser = URLPaser()
socket = urllib.urlopen("http://travel.sina.com.cn/quanbu-remen/")#打开这个网页

#fout = file('qq_art_urls.txt','w')#要把这个链接写到这个文件中
IParser.feed(socket.read())#分析啦

reg = 'http://travel.sina.com.cn/news/.*'#这个是用来匹配符合条件的链接，使用正则表达式匹配
...
for url in IParser.urls:#链接都存在urls里
    ...
    if pattern.match(url):
        ...
        if len(artical)<>0:
           i = i + 1
           f = open("sina/sina_travel/"+ str(i) + '.txt','a+')
           f.write(artical)
           f.close()
