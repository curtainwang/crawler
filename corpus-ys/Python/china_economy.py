...
IParser = URLPaser()
socket = urllib.urlopen("http://economy.china.com/hgjj/index.html")#打开这个网页
    ...
    if pattern.match(url):
        if url not in url2:
           # url = url.replace(".html","_all.html#page_2")
            ...
            if len(artical)<>0:
                  i = i + 1
                  f = open("china_news/Economy/"+str(i) + '.txt','a+')
                  f.write(artical)
                  f.close()

IParser = URLPaser()
socket = urllib.urlopen("http://economy.china.com/hgjj/index_1.html")#打开这个网页
IParser.feed(socket.read())#分析啦
reg = 'http://economy.china.com/hgjj/.*'#这个是用来匹配符合条件的链接，使用正则表达式匹配
pattern = re.compile(reg)
for url in IParser.urls:#链接都存在urls里
...
IParser = URLPaser()
socket = urllib.urlopen("http://economy.china.com/hgjj/index_2.html")#打开这个网页
IParser.feed(socket.read())#分析啦
reg = 'http://economy.china.com/hgjj/.*'#这个是用来匹配符合条件的链接，使用正则表达式匹配
pattern = re.compile(reg)
for url in IParser.urls:#链接都存在urls里
...
IParser = URLPaser()
socket = urllib.urlopen("http://economy.china.com/hgjj/index_3.html")#打开这个网页
IParser.feed(socket.read())#分析啦
reg = 'http://economy.china.com/hgjj/.*'#这个是用来匹配符合条件的链接，使用正则表达式匹配
pattern = re.compile(reg)
for url in IParser.urls:#链接都存在urls里
...
IParser = URLPaser()
socket = urllib.urlopen("http://economy.china.com/hgjj/index_15.html")#打开这个网页
IParser.feed(socket.read())#分析啦
reg = 'http://economy.china.com/hgjj/.*'#这个是用来匹配符合条件的链接，使用正则表达式匹配
pattern = re.compile(reg)
for url in IParser.urls:#链接都存在urls里
